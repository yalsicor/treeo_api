...
class Controller extends ApiController
{
    ...
    public function proxyLoginForUserMobileClient(LoginRequest $request)
    {
        $dataTransporter = new ProxyApiLoginTransporter(
            array_merge($request->all(), [
                'client_id'       => Config::get('authentication-container.clients.mobile.user.id'),
                'client_password' => Config::get('authentication-container.clients.mobile.user.secret')
            ])
        );
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function proxyRefreshForUserMobileClient(RefreshRequest $request)
    {
        $dataTransporter = new ProxyRefreshTransporter(
            array_merge($request->all(), [
                'client_id'       => Config::get('authentication-container.clients.mobile.user.id'),
